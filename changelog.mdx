...
<Update label="January 27" tags={["New releases", "Improvements"]} rss={{ title: "Draggable Photo Cards", description: "New drag-and-drop photo reordering, simplified location selection, and UI improvements" }}>
  ## Draggable Photo Cards

  Users can now **drag and drop profile photos** to reorder them on their profile. This provides an intuitive way to arrange photos and set your primary profile picture.

  ### New components

  - **DraggablePhotoCard**: A reusable card component with smooth drag animations powered by `react-native-reanimated` and `react-native-gesture-handler`
  - **DraggablePhotoGrid**: A container component that manages drag state and handles photo reordering

  ### Features

  - Smooth scale, rotation, and opacity animations during drag
  - Haptic feedback when dragging starts and when photos are swapped
  - Visual feedback showing drop targets
  - Automatic database sync when photos are reordered
  - Delete button overlay on each photo card

  ## Simplified Location Selection

  The location screen has been redesigned with a simpler multiple-choice city selection:

  - Pre-defined options for **New York City**, **Los Angeles**, and **Bay Area**
  - "My city isn't listed" option with custom city input and geocoding
  - Removed the interactive map interface for a cleaner, faster experience

  ## UI Improvements

  - Empty photo slots now display an image icon (`Ionicons.image-outline`) instead of a plain "+" symbol
  - Updated both the onboarding photo picker and picture review screens with the new icon style
  - Added Metro bundler optimizations with `inlineRequires: true` for better performance

  ## Technical details

  The draggable photo system uses:
  - `react-native-reanimated` for 60fps animations
  - `react-native-gesture-handler` for smooth gesture tracking
